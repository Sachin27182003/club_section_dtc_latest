...
export default async function SocietyHeadDashboard({ currentUser }: { currentUser: any }) {
    ...
    return (
      <div className="max-w-6xl mx-auto p-4 sm:p-6 space-y-8">
        ...
        <div className="flex flex-col items-center justify-center min-h-[50vh] text-center px-4 transition-colors">
          ...
          <p className="text-gray-500 dark:text-gray-400 mb-8 max-w-md">You are not an admin of any society yet. Create a new society to start managing events and members.</p>
          <Link href="/society/new" className="bg-blue-600 text-white px-6 py-3 rounded-lg font-medium hover:bg-blue-700 dark:bg-blue-500 dark:hover:bg-blue-600 transition-colors">
            + Create New Society
          </Link>
        </div>
      </div>
  ...
  const [upcomingEvents, previousEvents, pendingMemberRequests] = await Promise.all([
    ...
    <div className="max-w-6xl mx-auto p-4 sm:p-6 space-y-8 sm:space-y-12 transition-colors">
      ...
      <section className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-6 sm:p-8 shadow-sm flex flex-col lg:flex-row items-center lg:items-start justify-between gap-8 transition-colors">
        ...
        <div className="w-full lg:w-auto flex flex-col sm:flex-row gap-4 pt-6 lg:pt-0 border-t border-gray-100 dark:border-gray-800 lg:border-0 shrink-0">
          <Link href="/dashboard/requests" className="relative inline-flex items-center justify-center bg-amber-500 text-white px-6 py-3 rounded-lg font-medium hover:bg-amber-600 transition-colors shadow-sm w-full sm:w-auto">
            Pending Requests
            {headPendingCount > 0 && (
      ...
      <section>
        <div className="flex items-center justify-between mb-4 sm:mb-6">
          ...
          <Link href="/events/new" className="text-xs sm:text-sm bg-gray-900 dark:bg-white text-white dark:text-gray-900 px-3 py-2 sm:px-4 rounded-md hover:bg-gray-800 dark:hover:bg-gray-200 transition-colors">
            + New Event
          </Link>
        </div>
