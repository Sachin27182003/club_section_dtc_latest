...
async function CulturalSociety() {
  // 2. Fetch ALL societies
  const allClubs = await getAllClubs();

  // 3. Filter for just the Technical societies
  const culturalSocietyData = allClubs.filter(
    (club) => club?.type === "CULTURAL",
  );

  // Handle empty state
  if (!culturalSocietyData || culturalSocietyData.length === 0) {
    return (
